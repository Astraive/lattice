...
`lattice-uniffi` opens local profiles through a caller-supplied platform key protector. It exposes the public identity snapshot, a DER PKCS#10 request signed by the protected device key, exact full-fingerprint peer pins, bounded local Space Genesis pages with channel IDs, creation, text-message queueing and editing, and recent outgoing-history queries. The queue APIs revalidate an RFC 9420 X.509 credential, authorize against an unchanged local Genesis policy, and commit immutable signed events, outbox rows, and encrypted local history. Android Compose lists Genesis channels and provides a composer plus a latest-100 outgoing-history view per channel; locally authored messages can be selected for an immutable edit, with the encrypted cached body refreshed after the edit commits. History is local and does not include incoming events; network forwarding and recipient delivery remain unknown. No identity pin or queue result establishes Space membership.
