...
Android exposes Identity, Spaces and Diagnostics tabs; selecting a local Space opens its channel composer, while Diagnostics shows BLE, Wi-Fi and persistent-service readiness. The Compose content scrolls vertically, uses scalable Material text/input styles and exposes major sections as headings. Recovery and channel options use labeled radio semantics. Device TalkBack traversal and large-font behavior remain unverified while the available emulator is offline.
