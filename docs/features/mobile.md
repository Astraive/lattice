...
`lattice-uniffi` opens local profiles through a caller-supplied platform key protector. It exposes the public identity snapshot, a DER PKCS#10 request signed by the protected device key, exact full-fingerprint peer pins, bounded local Space Genesis pages with channel IDs, creation, text-message queueing, and recent outgoing-history queries. The queue API revalidates an RFC 9420 X.509 credential, authorizes against an unchanged local Genesis policy, and commits the signed event, outbox row, and encrypted local history. Android Compose lists Genesis channels and provides a composer plus a latest-100 outgoing-history view per channel. History is local and does not include incoming events; network forwarding and recipient delivery remain unknown. No identity pin or queue result establishes Space membership.
