...
The Android Rust library has been built for `arm64-v8a` and `x86_64` with `cargo-ndk`; `:app:compileDebugKotlin` and `:app:testDebugUnitTest` also completed successfully. Device behavior is unverified because `adb` is unavailable in the current workstation environment.
