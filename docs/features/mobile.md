...
Android Gradle builds regenerate the checked-in Kotlin bindings from the host UniFFI library with the Android cleaner enabled, then build Rust libraries for `arm64-v8a` and `x86_64`. `:app:compileDebugKotlin`, `:app:testDebugUnitTest`, `:app:assembleDebug`, and `:app:lintDebug` pass. The debug APK contains `liblattice_uniffi.so` for both ABIs. The APK was installed and launched on an API 37 Pixel 10 Pro emulator; this does not replace a physical-device lifecycle or hardware-backed-key matrix.
