...
`lattice-files` does not encrypt content, enforce membership, choose export paths, or provide a complete transfer workflow. The desktop client now supports OS-picker-based outbound selection, local source caching with 128 MiB/file, 512 MiB/64-source quotas, and authorized manifest queueing. Its cached source bytes are not encrypted at rest. Desktop does not yet send or receive attachment chunks, request incoming-file acceptance, or provide verified export. The disk store and `lattice-node` transport remain caller-integrated primitives; this is not completion of the FIL acceptance criteria.
