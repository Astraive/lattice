...
`lattice-files` does not encrypt content, enforce membership or global storage quotas, choose export paths, persist a staging store, or expire partial transfers. `lattice-node` adds authenticated, resumable attachment transport over an already connected peer adapter, but the caller must supply the authorized manifest/event, peer authorization, private staging storage, and retention/cleanup policy. The transfer receiver obtains explicit consent before staging chunks; export remains a caller action after verification to a safe selected destination. Resume reopens the same suitable seekable store and rechecks bytes rather than trusting a saved bitmap. This does not provide BLE/LAN discovery, platform path selection/failover, app UI, or Android/desktop adapter wiring.
