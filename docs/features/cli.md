...
The JSON `about.available` array includes `local_space_one_member_recovery`, `local_pinned_welcome_bootstrap_import`, and `local_peer_pin_revocation`; `remote_identity_revocation`, full invitation/leave and ongoing membership management, and network delivery remain unavailable.
