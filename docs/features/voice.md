...
- iOS is configured for iOS 17, `iphoneos` and `iphonesimulator`, iPhone and iPad. Supply a maintained, version-pinned WebRTC XCFramework with device and simulator slices and link it through the Xcode project. The project currently has no linked WebRTC framework/package or Rust iOS target integration. A usable path also needs the microphone usage declaration and `AVAudioSession` lifecycle/route handling.
