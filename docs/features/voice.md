...
Completing end-to-end voice requires selecting and integrating a WebRTC/secure-RTP backend and platform audio capture/output, permission, interruption, and device-route adapters; wiring authenticated Lattice signaling plus identity, membership, permission-epoch, and MLS-context verification; and implementing real ICE/STUN/TURN policy and route reporting. TURN additionally needs configured server credentials and deployment policy. A safe room-size limit requires a defined device/network test matrix and measured CPU, bandwidth, and audio results; none are available here. These are prerequisites, not behavior simulated by this crate. All VOC end-to-end and physical-device gates remain open.
