...
`Client::queue_text_message` accepts a restored local Space and credential; `queue_text_message_from_x509_credential` revalidates a bounded OS-trusted device certificate and restores only a valid, unchanged local Genesis generation. Both paths re-check channel policy, encrypt with MLS, and atomically commit the signed message, outbox envelope, encrypted local history record, author sequence, and sender state. `Client::queue_text_message_edit` can similarly commit a locally authorized Edit event referencing an authored message; the current in-memory reducer shows its version, but restart projection and cached history still expose only the original cached message. A successful return means **queued locally**, not forwarded or delivered. A later MLS membership transition or changed generation fails closed instead of using stale Genesis policy. Authenticated incoming event processing, full policy/history replay, deletion, reactions, sync, and delivery receipts remain unsupported; Edit authoring is not yet exposed by UniFFI or application UI.
