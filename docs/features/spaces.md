...
The current Rust core can create a one-member candidate generation locally, atomically persist its signed Genesis event and encrypted initial-policy snapshot, enumerate local snapshots in bounded 32-entry keyset pages, and restore each generation only after rechecking the event, protected MLS group and snapshot authentication. Recovery covers Genesis only: later policy replay, credential trust, membership transitions, conflict restoration, joins and message authoring remain unavailable.
