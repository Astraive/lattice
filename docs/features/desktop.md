...
The current desktop prototype protects device identity, exposes exact full-fingerprint peer pin/lookup, and provides a bounded read-only listing of local Genesis snapshots through Tauri. Listing verifies local event and snapshot integrity; it does not establish current membership. Space creation/join, authenticated messaging, discovery, relay configuration, persistent courier behavior and release-matrix verification remain incomplete.
