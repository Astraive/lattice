...
`lattice-router` supplies a deterministic bounded planner; it keeps Presence off persistent Courier/InternetRelay routes and Voice on one direct realtime-IP route. `lattice-node::sync` adds `execute_authenticated_sync_once` and `serve_authenticated_sync_request_once` over a caller-selected, already-connected `TransportAdapter`. Noise XX protects frames, transcript-bound Ed25519 proofs bind the peer to a caller-supplied `PinnedIdentity`, and the scope callback runs before request planning or source access.
`StoreSyncEventSource` can serve only committed, signature-verified records from one caller-selected Space/group generation; storage and metadata failures abort the exchange rather than masquerading as unavailable events. Peers derive the opaque scope with `space_generation_scope_id(space_id, group_reference)`, domain-separated over both fixed-width identifiers. CLI `sync serve-once` accepts one previously pinned peer on caller-selected TCP and serves stored records without claiming recipient delivery or event application. CLI `sync fetch-once` requests one caller-selected event ID and passes validated application events through Core's atomic MLS/Space acceptance boundary. These commands do not discover peers, select adapters through `lattice-router`, integrate the outbox, provide full summary reconciliation, or supply relay sync-control. Transport receipts remain exact-hop only.
...
`lattice-sync::plan_sync` performs bounded per-scope range reconciliation and rejects conflicting known event IDs at a shared author sequence. The authenticated direct APIs request at most 128 exact event-ID/sequence targets and receive at most one response; they validate the caller-selected scope before accessing event sources. Missing, rejected, unavailable, or frame-omitted records remain explicit unresolved requests. TCP receipts mean local OS queue acceptance, not remote delivery.
Returned event bytes remain opaque until a caller validator accepts them, then the inner event ID is deduplicated. The transport does not commit or apply events, classify MLS dependency types, process Commits, or refresh summaries. `Client::accept_synced_application_event` provides an atomic Core boundary for one exact received application event and retains events with missing signed parents in bounded pending storage. CLI fetch-once calls that boundary for an exact event-ID request; it does not refresh summaries or process MLS Commits, so dependencies outside that one event remain unresolved for a later request. The legacy unauthenticated exchange API is removed. NET-012's full anti-entropy and relay sync-control path remain unimplemented.
