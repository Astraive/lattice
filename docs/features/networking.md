...
`lattice-transport::TcpPeerAdapter` provides bounded 4-byte big-endian framing over one caller-selected TCP stream; it checks lengths before allocation, treats cancellation during an incomplete send as a fault, and reports only local OS queue acceptance. It does not authenticate peers, discover LAN devices, or provide retry/delivery semantics. Consequently, it is a transport primitive, not evidence that NET-006 capability negotiation or an authenticated LAN path is complete.
