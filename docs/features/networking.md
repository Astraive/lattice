...
The shared protocol crate now defines a bounded, canonical version-1 offer for LAN, Wi-Fi Aware and Wi-Fi Direct/P2P frame limits. Peers intersect offers using the smaller limit per common path; this reports supported sizes only and never claims reachability. The codec is not yet integrated into authenticated BLE discovery or platform path adapters, so NET-006 remains incomplete.
