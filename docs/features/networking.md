...
`StoreSyncEventSource` can serve only committed, signature-verified records from one caller-selected Space/group generation; storage and metadata failures abort the exchange rather than masquerading as unavailable events. Peers derive the opaque scope with `space_generation_scope_id(space_id, group_reference)`, domain-separated over both fixed-width identifiers. CLI `sync serve-once` accepts one previously pinned peer on caller-selected TCP, serves stored records for that generation, and makes no recipient-delivery or event-application claim. Direct sync still requires caller-supplied same-scope summaries; it does not discover peers, select adapters through `lattice-router`, persist or apply received events, manage MLS dependencies, integrate the outbox, or provide a relay sync-control path. Transport receipts remain exact-hop only.
...
Returned event bytes remain opaque until a caller validator accepts them, then the inner event ID is deduplicated. The transport does not commit or apply events, classify MLS dependency types, process Commits, or refresh summaries; callers must resolve known dependencies before replanning. The legacy unauthenticated exchange API is removed. NET-012's end-to-end MLS transition and relay sync-control path remain unimplemented.
