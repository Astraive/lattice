...
Nostr adapter wraps already protected Lattice envelopes in a signed outer event. It needs a defined event kind, retrieval tag, outer key, size limit, expiration and backfill window under ADR-003. Relays are chosen locally, may be multiple, and may disagree on acceptance or retention. Encryption does not hide client IP, relay subscriptions, timing or payload lengths. A relay outage leaves local Space state intact; a previously published envelope can arrive again over BLE with the same event ID.
