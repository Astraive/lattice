...
`lattice-router` supplies a deterministic bounded planner; it keeps Presence off persistent Courier/InternetRelay routes and Voice on one direct realtime-IP route. `lattice-node::sync` now joins `lattice-sync` planning to a one-request/one-response exchange over an already-running `TransportAdapter`, including `TcpPeerAdapter`'s bounded direct TCP records. This caller-driven path does not use router path selection, authenticate peers, or integrate an outbox.
...
`lattice-sync::plan_sync` performs bounded per-scope range reconciliation and rejects conflicting known event IDs at a shared author sequence. `lattice-node::sync::execute_sync_once` plans from same-scope summaries, sends at most 128 exact event-ID/sequence requests, and receives at most one response; `serve_sync_request_once` checks the caller-selected scope and returns only exact requested records that fit the adapter's frame cap. Unanswered, cancelled, rejected, or unavailable items remain explicit unresolved requests. Direct receipts retain the adapter's exact scope (TCP reports local OS queue acceptance), not remote delivery.
Sync event bytes stay opaque through the wire codec. The caller validator is invoked before deduplication, which keys on the returned inner event ID; this path does not commit records, identify MLS dependency types, validate commits, or schedule refreshed summaries. Dependency IDs are requested before sequence ranges as enforced by `plan_sync`; callers must resolve them and replan. The exchange is one-shot and caller-cancelled, with no retries, subscriptions, persistence, authentication, or relay sync-control path. NET-012's end-to-end MLS transition remains unimplemented.
