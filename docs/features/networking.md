...
This direct session is one-shot and requires caller-supplied same-scope summaries. It does not discover peers, select adapters through `lattice-router`, persist or apply received events, manage MLS dependencies, integrate the outbox, or provide a relay sync-control path. Transport receipts remain exact-hop only.
