...
`StoreSyncEventSource` can serve only committed, signature-verified records from one caller-selected Space/group generation; storage and metadata failures abort the exchange rather than masquerading as unavailable events. Direct sync still requires caller-supplied same-scope summaries. It does not discover peers, select adapters through `lattice-router`, persist or apply received events, manage MLS dependencies, integrate the outbox, or provide a relay sync-control path. Transport receipts remain exact-hop only.
